...
@SpringBootTest(
        classes = ServiceHealthLogRepositorySmokeTest.TestConfig.class,
        properties = {
                "spring.datasource.url=jdbc:h2:mem:geihou_infra_service_health_log_repo_test;DB_CLOSE_DELAY=-1;MODE=MySQL",
                "spring.datasource.driver-class-name=org.h2.Driver",
                "spring.datasource.username=sa",
                "spring.datasource.password="
        }
)
class ServiceHealthLogRepositorySmokeTest {

    @Configuration
    @EnableAutoConfiguration
    @Import({GeihouMyBatisAutoConfiguration.class, ServiceHealthLogRepository.class})
    ...
    void shouldInsertAndSelectById() {
        ...
        assertThat(repository.insert(log)).isEqualTo(1);
        assertThat(log.getId()).isNotNull();

        ServiceHealthLogDO selected = repository.selectById(log.getId());
    ...
    void shouldSelectOneBySingleField() {
        ServiceHealthLogDO log = repository.selectOne(
    ...
    void shouldSelectListsAndCounts() {
        assertThat(repository.selectList()).hasSize(3);
        assertThat(repository.selectList(ServiceHealthLogDO::getStatus, "UP")).hasSize(2);
        assertThat(repository.selectList(ServiceHealthLogDO::getStatus, "DOWN",
                ServiceHealthLogDO::getServiceName, "geihou-module-report")).hasSize(1);
        assertThat(repository.selectCount(ServiceHealthLogDO::getStatus, "DOWN")).isEqualTo(1L);
    }

    @Test
    void shouldSelectPages() {
        PageResult<ServiceHealthLogDO> firstPage = repository.selectPage(1, 2);
        ...
        PageResult<ServiceHealthLogDO> orderedPage = repository.selectPage(1, 2,
        ...
        PageResult<ServiceHealthLogDO> upPage = repository.selectPage(
                ServiceHealthLogDO::getStatus, "UP", 1, 10);
        assertThat(upPage.getList()).hasSize(2);
        assertThat(upPage.getTotal()).isEqualTo(2L);

        PageResult<ServiceHealthLogDO> downPage = repository.selectPage(
